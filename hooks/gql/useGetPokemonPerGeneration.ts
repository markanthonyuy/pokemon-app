...
export const useGetPokemonPerGeneration = ({ name, order = 'asc' }: UseGetPokemonPerGenerationProps) => {
  const { data, error, loading, fetchMore, networkStatus } = useQuery(GET_GENERATION_QUERY, {
    variables: {
      name,
      order,
      limit: PAGE_OFFSET,
  ...
  const totalCount =
    data?.pokemon_v2_generation[0]?.pokemon_v2_pokemonspecies_aggregate?.aggregate?.count || 0
    ...
    try {
      fetchMore({
        ...
        updateQuery: (prev, { fetchMoreResult }) => {
          ...
          setPokemons([...allPokemons, ...fetchMoreResult.pokemon_v2_generation[0].pokemon_v2_pokemonspecies]);
          return {
            ...prev,
            pokemon_v2_generation: [
              {
                ...
                pokemon_v2_pokemonspecies: [
                  ...
                  ...fetchMoreResult.pokemon_v2_generation[0].pokemon_v2_pokemonspecies
                ]
              }
            ]
          };
  ...
  return { pokemons: allPokemons, error, loading, loadMore, networkStatus, offset, totalCount };
}